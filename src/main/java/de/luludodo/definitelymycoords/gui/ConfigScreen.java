...
public class ConfigScreen extends Screen {
    ...
    private boolean oldObscureRotations = true;
    ...
    protected void init() {
        ...
        obscureRotations = SimpleOption.ofBoolean("options.definitelymycoords.obscure-rotations", ConfigAPI.getObscureRotations(), value -> {
            if (ConfigAPI.getObscureRotations()) {
        ...
        elements[1] = addDrawableChild(obscureRotations.createButton(settings, width / 2 - 100, height / 6 - 16, 200));
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (client.world == null) {
            ...
            client.inGameHud.debugHud.render(matrices);
            matrices.pop();
            matrices.push();
            matrices.translate(0, 0, -100);
            renderCustomBackground(matrices);
        }
        ...
        if (mode.getValue() != Mode.CUSTOM && mode.getValue() != Mode.VANILLA) {
            drawOffsetText(matrices, offsetX, "X", 38);
            drawOffsetText(matrices, offsetY, "Y", 62);
            drawOffsetText(matrices, offsetZ, "Z", 86);
        }
        drawChunksMisalignedWarningTextIfNeeded(matrices);
        drawBiomeText(matrices, (TextFieldWidget) elements[6]);
    ...
    private void infoMode(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (chunksMisalignedWarningTextHovered(mouseX, mouseY)) {
            renderTooltip(matrices, "chunks-misaligned-warning", mouseX, mouseY);
            return;
        }
    ...
    private void renderTooltip(MatrixStack matrices, String name, int x, int y) {
        renderTooltip(matrices, List.of(Text.translatable("tooltip.definitelymycoords." + name)), x, y);
    }

    private void drawTitle(MatrixStack matrices) {
        drawCenteredText(matrices, title, width / 2, height / 6 - 30, 0xFFFFFF);
    }

    private void drawOffsetText(MatrixStack matrices, TextFieldWidget offset, String name, int heightOffset) {
        int V = validNumber(offset.getText());
        int color = V == 0? 0xFFFFFF: V == 1? 0x00FF00: 0xFF0000;
        drawCenteredText(matrices, name, width / 2 - 88, height / 6 + heightOffset, color);
    }

    private void drawBiomeText(MatrixStack matrices, TextFieldWidget biome) {
        int valid = validBiome(client, biome.getText());
        int color = valid == 0? 0xFFFFFF: valid == 1? 0x00FF00: 0xFF0000;
        drawCenteredText(matrices, "Biome", width / 2 - 80, height / 6 + 126, color);
    }
    ...
    private void drawChunksMisalignedWarningTextIfNeeded(MatrixStack matrices) {
        if (chunksAligned())
            return;

        drawCenteredText(matrices, Text.translatable("options.definitelymycoords.chunks-misaligned-warning"), width / 2, height / 6 + 105, 0xFFFF00);
    }
    ...
    private void drawCenteredText(MatrixStack matrices, String text, int centerX, int y, int color) {
    ...
    private void renderCustomBackground(MatrixStack matrices) {
        int x1 = width / 2 - 104;
        int y1 = height / 6 - 34;
        int x2 = x1 + 208;
        int y2 = y1 + 228;
        fill(matrices, x1, y1, x2, y2, 0xAA000000);

        int x3 = x1 - 1;
        int y3 = y1 - 1;
        int x4 = x2 + 1;
        int y4 = y2 + 1;
        fill(matrices, x3, y3, x4, y1, 0xFFFFFFFF);
        fill(matrices, x3, y2, x4, y4, 0xFFFFFFFF);
        fill(matrices, x3, y3, x1, y4, 0xFFFFFFFF);
        fill(matrices, x2, y3, x4, y4, 0xFFFFFFFF);
    }
}